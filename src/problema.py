...
class Estado:
    def __init__(self, veiculos, pedidos_pendentes, tempo_atual=0, custo_acumulado=0, alerta=None, arestas_transito=None):
    ...
    def copia_segura(self):
        ...
        # Passamos também o alerta (embora normalmente seja limpo no sucessor)
            ...
            if not veiculo.ocupado:
                for pedido in self.pedidos_pendentes:
                    if veiculo.local == pedido.origem:
                        ...
                        ns.alerta = None # Limpar alertas antigos no novo estado
    ...
    def __hash__(self):
        v_info = tuple((v.id, v.local, v.ocupado, int(v.autonomia_atual // 10)) for v in self.veiculos)
